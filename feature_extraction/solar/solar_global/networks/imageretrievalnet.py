...
def extract_vectors(cuda_devices, net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, summary=None, mode='test'):
    # moving network to gpu and eval mode
    net.cuda(cuda_devices)
    net.eval()
...
def extr_selfmade_dataset(net, selfmadedataset, image_size, transform, ms=[1], msp=1, NoGPU=False):
    if selfmadedataset == 'GLM/test':
        ...
        folder_path = os.path.join('/mnt/c/Users/User/Desktop/', selfmadedataset)
        images, img_r_path = path_all_jpg(folder_path, start="/mnt/c/Users/User/Desktop/")
        print(folder_path)
...
def extract_vectors_orig(net, root, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, summary=None, mode='test'):
    # moving network to gpu and eval mode
    net.cuda()
    net.eval()
    ...
    # creating dataset loader
    loader = torch.utils.data.DataLoader(
            ImagesFromList(root=root, images=images, imsize=image_size, bbxs=bbxs, transform=transform, mode=mode),
            batch_size=1, shuffle=False, num_workers=8, pin_memory=True
        )
