...
def load_network(cuda_device, network_name='resnet101-solar-best.pth'):
    # loading network
    # pretrained networks (downloaded automatically)
    print(">> Loading network:\n>>>> '{}'".format(network_name))
    # state = torch.load(os.path.join(get_data_root(), 'networks', network_name))
    state = torch.load(os.path.join(get_data_root(), 'networks', network_name), map_location=torch.device('cpu'))
